...
function renderSurfaceState(){
  ...
  if(surface.id==='contracts'){
    ...
    out.innerHTML='<div class="surface-card"><strong>Reality Lens Ω</strong><span>semantic zoom · 2-hand VIDEO mode · air keyboard · finger navigation</span></div><div class="surface-row"><strong>Camera</strong><span>'+((workspace.state.lens.cameraEnabled)?'EXPLICITLY ON':'OFF BY DEFAULT')+'</span></div><div class="surface-row"><strong>Input engines</strong><span>'+workspace.input.airTyping.getState().schemaVersion+' · '+(workspace.input.handLens.enabled?'ACTIVE':'READY')+'</span></div><button id="camera-toggle">'+(workspace.state.lens.cameraEnabled?'Disable camera':'Enable camera')+'</button><div class="overlay-actions"><button id="air-demo">Air-type A</button><button id="hand-demo">Simulate pinch</button></div>';
    $('#camera-toggle').onclick=()=>{workspace.toggleCamera(!workspace.state.lens.cameraEnabled);renderAll()};
    $('#air-demo').onclick=()=>{workspace.input.airTyping.setKeyMap([{key:'a',rect:{x:0,y:0,w:1,h:1}}]);workspace.input.airTyping.registerTap(1,.5,.5,0);const event=workspace.input.airTyping.poll(100)[0];$('#console-output').textContent=pretty({surface:'Reality Lens Ω',airTyping:event});};
    $('#hand-demo').onclick=()=>{const hands=[Array.from({length:21},()=>({x:.5,y:.5,z:0}))];hands[0][4]={x:.5,y:.5,z:0};hands[0][8]={x:.52,y:.52,z:0};workspace.input.handLens.setEnabled(true);const sample=workspace.input.handLens.update(hands);$('#console-output').textContent=pretty({surface:'Reality Lens Ω',handEvent:sample.events[0]});};
